...
// Map endpoints to ID column names
const endpointToIdColumnMap = {
  'editoras': 'ed_cod',
  'autores': 'au_cod',
  'codigos-postais': 'cod_postal',
  'generos': 'ge_genero',
  'livros': 'li_cod',
  'exemplares': 'lex_cod',
  'utentes': 'ut_cod',
...
const getTableNameFromEndpoint = (endpoint) => {
  // Remove leading/trailing slashes and split
  const parts = endpoint.replace(/^\/+|\/+$/g, '').split('/').filter(p => p);
  const endpointName = parts[0]; // First part is the endpoint name
  const tableName = endpointToTableMap[endpointName];
  if (!tableName) {
    console.warn(`No table mapping found for endpoint: ${endpointName}, using as-is`);
    return endpointName;
  }
  return tableName;
};

// Helper function to get ID column from endpoint
const getIdColumnFromEndpoint = (endpoint) => {
  // Remove leading/trailing slashes and split
  const parts = endpoint.replace(/^\/+|\/+$/g, '').split('/').filter(p => p);
  const endpointName = parts[0]; // First part is the endpoint name
  return endpointToIdColumnMap[endpointName] || 'id';
};
  ...
  get: async (endpoint) => {
    if (finalDatabaseType === 'supabase') {
      try {
        ...
        const result = await supabaseQueries.getAll(table, {
          order: { column: getDefaultOrderColumn(table), ascending: true }
        });
        // Return in the same format as the API (wrapped in data property)
  ...
  post: async (endpoint, data) => {
    if (finalDatabaseType === 'supabase') {
      try {
        // Handle Supabase direct calls
        const table = getTableNameFromEndpoint(endpoint);
        const result = await supabaseQueries.insert(table, data);
        return { data: Array.isArray(result) ? result[0] : result, success: true };
      } catch (error) {
        console.error(`Error creating in ${endpoint}:`, error);
        throw new Error(error.message || 'Erro ao criar registo');
      }
    } else {
      // Use traditional API
  ...
  put: async (endpoint, data) => {
    if (finalDatabaseType === 'supabase') {
      try {
        ...
        const table = getTableNameFromEndpoint(endpoint);
        const idColumn = getIdColumnFromEndpoint(endpoint);
        const id = parts[parts.length - 1]; // Last part is the ID
        const filters = {};
        filters[idColumn] = id;
        const result = await supabaseQueries.update(table, data, filters);
        return { data: Array.isArray(result) ? result[0] : result, success: true };
      } catch (error) {
        console.error(`Error updating in ${endpoint}:`, error);
        throw new Error(error.message || 'Erro ao atualizar registo');
      }
    } else {
      // Use traditional API
  ...
  delete: async (endpoint) => {
    if (finalDatabaseType === 'supabase') {
      try {
        ...
        const filters = {};
        filters[idColumn] = id;
        await supabaseQueries.delete(table, filters);
        return { success: true };
      } catch (error) {
        console.error(`Error deleting from ${endpoint}:`, error);
        throw new Error(error.message || 'Erro ao eliminar registo');
      }
    } else {
      // Use traditional API
...
// Helper function to get default order column for each table
const getDefaultOrderColumn = (table) => {
  const orderMap = {
    'editora': 'ed_nome',
    'autor': 'au_nome',
    ...
    'genero': 'ge_genero',
    'livro': 'li_titulo',
    'livro_exemplar': 'lex_cod',
    'utente': 'ut_nome',
    'requisicao': 're_data_requisicao'
  };
  return orderMap[table] || 'id';
};
